//
//  LPDefaultPlayViewController.m
//  LPMusicKitDemo
//
//  Created by lyr on 2020/5/20.
...
@interface LPDefaultPlayViewController ()<TuneInProgressSliderDelegate, TuneInVolumeSliderDelegate,TuneInVolumeSliderDatasource>
/**
 UI
 */
...
@implementation LPDefaultPlayViewController
...
- (void)viewDidLoad {
    ...
    [self.popImageView addGestureRecognizer:recognizer];
...
//            }
    ...
    [self.KVOController observe:self.boxInfo.mediaInfo keyPath:@"ratingURI" options:NSKeyValueObservingOptionInitial|NSKeyValueObservingOptionNew|NSKeyValueObservingOptionOld block:^(id observer, id object, NSDictionary *change) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            {
                if (!self.thumbUpButton.isHidden){
                    self.thumbUpButton.hidden = self.thumbDownButton.hidden = YES;
                }
                return ;
            }
            ...
            if (![thumbInfo[@"url"] isEqualToString:url]){
                [self requestStationThumbState:url];
            }
            else if ([thumbInfo[@"url"] isEqualToString:url] && self.thumbUpButton.isHidden)
            {
                self.thumbUpButton.hidden = self.thumbDownButton.hidden = NO;
            }
            [self setThumUpAndDownStatu:[thumbInfo[@"thumbRating"] intValue]];
...
- (void)setThumUpAndDownStatu:(int)statu
{
    ...
    if (statu == 2){
        [self.thumbDownButton setImage:[self imageNamed:@"muzo_playam_hate_n"] forState:UIControlStateNormal];
        [self.thumbUpButton setImage:[self imageNamed:@"muzo_playam_like_d"] forState:UIControlStateNormal];
    }else if (statu == 1){
        [self.thumbDownButton setImage:[self imageNamed:@"muzo_playam_hate_d"] forState:UIControlStateNormal];
        [self.thumbUpButton setImage:[self imageNamed:@"muzo_playam_like_n"] forState:UIControlStateNormal];
    }else{
        [self.thumbDownButton setImage:[self imageNamed:@"muzo_playam_hate_n"] forState:UIControlStateNormal];
        [self.thumbUpButton setImage:[self imageNamed:@"muzo_playam_like_n"] forState:UIControlStateNormal];
